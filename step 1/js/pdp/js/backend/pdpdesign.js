...
mstPDPPopup(function($){
	...
	var setupAreaUrl = $("#request_url").val();
	...
	PDPPopup = {
		...
		createSample : function () {
			...
			$(".create-sample-btn").on("click", function() {
				PDPPopup.openPopup(url, "Create Sample Design", PDPPopup.setupAreaHandler );
			});
		}(),
		...
		reOrderAdditionalInfo : function() {
			...
			$(".order-tables .item-container").each(function() {
				...
				if ($(this).find(".item-options").length) {
					$(this).find(".item-options").prepend(customizeLink);
				} else if ($(this).find(".item-text").length) {
					$(this).find(".item-text").append(customizeLink);
				}
			});
		}()
	}
