...
class MST_Pdp_Adminhtml_DesignareaController extends Mage_Adminhtml_Controller_Action
{
		...
		if ($data) {
            ...
            try {
				...
				if ( $valid == false )  {
                    ...
                    $url = Mage::helper("adminhtml")->getUrl("pdp/adminhtml_designarea/addNewSide/", array ("productid" => $productId));
	...
	public function saveDesignInlayAction() {
		...
		if ($filename != "") {
			...
			if ($inlayId != "") {
				...
				if ($itemId != "") {
					Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('pdp')->__('Item was successfully saved'));
					$url = Mage::helper("adminhtml")->getUrl("pdp/adminhtml_designarea/setuparea/",array("productid"=> $_productId, "areaid" => $_sideId ));
	...
	public function saveDesignColorAction() {
		...
		Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('pdp')->__('Item was successfully saved'));
		$url = Mage::helper("adminhtml")->getUrl("pdp/adminhtml_designarea/viewdesigncolor/",array("productid"=> $productColorInfo['product_id'] ));
		$this->getResponse()->setRedirect($url);
	...
	public function updateDesignColorAction() {
		...
		Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('pdp')->__('Item was successfully saved'));
		$url = Mage::helper("adminhtml")->getUrl("pdp/adminhtml_designarea/viewdesigncolor/",array("productid"=> $data['product_id'] ));
		$this->getResponse()->setRedirect($url);
	}
